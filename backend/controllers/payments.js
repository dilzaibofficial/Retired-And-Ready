...
const { paymentPaid } = require("../mail/templates/PaymentPaid");
const stripe = Stripe(process.env.STRIPE_SECRET_KEY);

exports.capturePayment = async (req, res) => {
  const { coursesId } = req.body;
  const userId = req.user.id;
  console.log("important check 1", coursesId);
  console.log("important check 2", userId);
  ...
  try {
    for (const course_id of coursesId) {
      ...
      console.log("buying course detail", course);
      ...
      lineItems.push({
        price_data: {
          ...
          unit_amount: course.price * 100, // Stripe expects amount in paisa
        },
        quantity: 1,
      });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `http://localhost:5173/success?user_id=${userId}&course_id=${coursesId}&amount=${totalAmount}`,
...
exports.verifyPayment = async (req, res) => {
  console.log("making me cry", req.body);
...
exports.sendPaymentSuccessEmail = async (req, res) => {
  ...
  try {
    ...
    let fullname = `${enrolledStudent.firstName} ${enrolledStudent.lastName}`;
    const result = await mailSender(
      enrolledStudent.email,
      `Payment Recieved`,
      paymentPaid(fullname, amount)
    );
    if (result.success) {
